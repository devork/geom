...
func TestGeometryCollection(t *testing.T) {
	...
	for _, dataset := range datasets {
		...
		assert.Equal(t, dataset.dim, gcol.Dimension(), "Expected dim %v, but got %v ", dataset.dim, g.Dimension)
		assert.Equal(t, dataset.srid, gcol.SRID(), "Expected srid %v, but got %v ", dataset.srid, g.SRID())

		assert.Equal(t, 2, len(gcol.Geometries))

		point := gcol.Geometries[0].(*geom.Point)

		assert.InDelta(t, 4, point.Coordinate[0], 1e-9)
		assert.InDelta(t, 6, point.Coordinate[1], 1e-9)

		lstring := gcol.Geometries[1].(*geom.LineString)
...
func TestMultiPolygon(t *testing.T) {
	...
	for _, dataset := range datasets {
		...
		assert.Equal(t, dataset.dim, mpolygon.Dimension(), "Expected dim %v, but got %v ", dataset.dim, g.Dimension())
		assert.Equal(t, dataset.srid, mpolygon.SRID(), "Expected srid %v, but got %v ", dataset.srid, g.SRID())
...
func TestMultiLineString(t *testing.T) {
	datasets := []struct {
		...
		{"", 0, geom.XY},             // wkb_hdr
		{"", 0, geom.XY},             // wkb_xdr
	}
	...
	for _, dataset := range datasets {
		...
		assert.Equal(t, dataset.dim, mlstring.Dimension(), "Expected dim %v, but got %v ", dataset.dim, g.Dimension())
		assert.Equal(t, dataset.srid, mlstring.SRID(), "Expected srid %v, but got %v ", dataset.srid, g.SRID())
...
func TestMultiPoint(t *testing.T) {
	datasets := []struct {
		...
		{"002000000400006c340000000400000000014024000000000000404400000000000000000000014044000000000000403e0000000000000000000001403400000000000040340000000000000000000001403e0000000000004024000000000000", 27700, geom.XY}, // ewkb_xdr
		{"0104000020346c000004000000010100000000000000000024400000000000004440010100000000000000000044400000000000003e4001010000000000000000003440000000000000344001010000000000000000003e400000000000002440", 27700, geom.XY}, // ewkb_hdr
		{"00000000040000000400000000014024000000000000404400000000000000000000014044000000000000403e0000000000000000000001403400000000000040340000000000000000000001403e0000000000004024000000000000", 0, geom.XY},             // wkb_hdr
		{"010400000004000000010100000000000000000024400000000000004440010100000000000000000044400000000000003e4001010000000000000000003440000000000000344001010000000000000000003e400000000000002440", 0, geom.XY},             // wkb_xdr
	}
	...
	for _, dataset := range datasets {
		...
		assert.Equal(t, dataset.dim, mpoint.Dimension(), "Expected dim %v, but got %v ", dataset.dim, g.Dimension())
		assert.Equal(t, dataset.srid, mpoint.SRID(), "Expected srid %v, but got %v ", dataset.srid, g.SRID())
...
func TestLineString(t *testing.T) {
	datasets := []struct {
		...
		{"002000000200006c3400000003403e00000000000040240000000000004024000000000000403e00000000000040440000000000004044000000000000", 27700, geom.XY}, // ewkb_xdr
		{"0102000020346c0000030000000000000000003e40000000000000244000000000000024400000000000003e4000000000000044400000000000004440", 27700, geom.XY}, // ewkb_hdr
		{"000000000200000003403e00000000000040240000000000004024000000000000403e00000000000040440000000000004044000000000000", 0, geom.XY},             // wkb_hdr
		{"0102000000030000000000000000003e40000000000000244000000000000024400000000000003e4000000000000044400000000000004440", 0, geom.XY},             // wkb_xdr
	}
	...
	for _, dataset := range datasets {
		...
		assert.Equal(t, dataset.dim, lstring.Dimension(), "Expected dim %v, but got %v ", dataset.dim, g.Dimension)
		assert.Equal(t, dataset.srid, lstring.SRID(), "Expected srid %v, but got %v ", dataset.srid, g.SRID())
		assert.Equal(t, 3, len(lstring.Coordinates))
...
func TestDimensionsAndEndian(t *testing.T) {
	...
	datasets := []struct {
		...
		// XY
		{"00000000013ff00000000000003ff0000000000000", 0, geom.XY, []float64{1, 1}}, // EWKB XDR
		{"0101000000000000000000f03f000000000000f03f", 0, geom.XY, []float64{1, 1}}, // EWKB HDR
		{"00000000013ff00000000000003ff0000000000000", 0, geom.XY, []float64{1, 1}}, // WKB XDR
		{"0101000000000000000000f03f000000000000f03f", 0, geom.XY, []float64{1, 1}}, // WKB HDR

		// XYS
		{"002000000100006c343ff00000000000003ff0000000000000", 27700, geom.XY, []float64{1, 1}}, // EWKB XDR
		{"0101000020346c0000000000000000f03f000000000000f03f", 27700, geom.XY, []float64{1, 1}}, // EWKB HDR
		{"00000000013ff00000000000003ff0000000000000", 0, geom.XY, []float64{1, 1}},             // WKB XDR
		{"0101000000000000000000f03f000000000000f03f", 0, geom.XY, []float64{1, 1}},             // WKB HDR

		// XYZ
		{"00800000013ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYZ, []float64{1, 1, 1}}, // EWKB XDR
		{"0101000080000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYZ, []float64{1, 1, 1}}, // EWKB HDR
		{"00000003e93ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYZ, []float64{1, 1, 1}}, // WKB XDR
		{"01e9030000000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYZ, []float64{1, 1, 1}}, // WKB HDR

		// XYZS
		{"00a000000100006c343ff00000000000003ff00000000000003ff0000000000000", 27700, geom.XYZ, []float64{1, 1, 1}}, // EWKB XDR
		{"01010000a0346c0000000000000000f03f000000000000f03f000000000000f03f", 27700, geom.XYZ, []float64{1, 1, 1}}, // EWKB HDR
		{"00000003e93ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYZ, []float64{1, 1, 1}},             // WKB XDR
		{"01e9030000000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYZ, []float64{1, 1, 1}},             // WKB HDR

		// XYM
		{"00400000013ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYM, []float64{1, 1, 1}}, // EWKB XDR
		{"0101000040000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYM, []float64{1, 1, 1}}, // EWKB HDR
		{"00000007d13ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYM, []float64{1, 1, 1}}, // WKB XDR
		{"01d1070000000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYM, []float64{1, 1, 1}}, // WKB HDR

		// XYMS
		{"006000000100006c343ff00000000000003ff00000000000003ff0000000000000", 27700, geom.XYM, []float64{1, 1, 1}}, // EWKB XDR
		{"0101000060346c0000000000000000f03f000000000000f03f000000000000f03f", 27700, geom.XYM, []float64{1, 1, 1}}, // EWKB HDR
		{"00000007d13ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYM, []float64{1, 1, 1}},             // WKB XDR
		{"01d1070000000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYM, []float64{1, 1, 1}},             // WKB HDR

		// XYZM
		{"00c00000013ff00000000000003ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYZM, []float64{1, 1, 1, 1}}, // EWKB XDR
		{"01010000c0000000000000f03f000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYZM, []float64{1, 1, 1, 1}}, // EWKB HDR
		{"0000000bb93ff00000000000003ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYZM, []float64{1, 1, 1, 1}}, // WKB XDR
		{"01b90b0000000000000000f03f000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYZM, []float64{1, 1, 1, 1}}, // WKB HDR

		// XYZM
		{"00e000000100006c343ff00000000000003ff00000000000003ff00000000000003ff0000000000000", 27700, geom.XYZM, []float64{1, 1, 1, 1}}, // EWKB XDR
		{"01010000e0346c0000000000000000f03f000000000000f03f000000000000f03f000000000000f03f", 27700, geom.XYZM, []float64{1, 1, 1, 1}}, // EWKB HDR
		{"0000000bb93ff00000000000003ff00000000000003ff00000000000003ff0000000000000", 0, geom.XYZM, []float64{1, 1, 1, 1}},             // WKB XDR
		{"01b90b0000000000000000f03f000000000000f03f000000000000f03f000000000000f03f", 0, geom.XYZM, []float64{1, 1, 1, 1}},             // WKB HDR
	}
	...
	for _, dataset := range datasets {
		...
		assert.Equal(t, dataset.dim, point.Dimension(), "Expected dim %s, but got %s", dataset.dim, g.Dimension())
		assert.Equal(t, dataset.srid, point.SRID(), "Expected srid %v, but got %v ", dataset.srid, g.SRID())
